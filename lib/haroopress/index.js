...
//    var archiveFiles = direc.sort(conf.sourceDir +'/articles', 'mtime'),
    ...
    function initialize() {
        ...
        authorFiles.forEach(function(file) {
            ...
            author.body = mmd.render(author.body);
            author.archives = [];
            authors[author.head.name] = author;
        });

        var archive, id, idx = 0, arr = [];
        archiveFiles.forEach(function(file, i) {
            id = getFileName(file);
            ...
            archive.html = tempBody.length < 5 ? '' : mmd.render(tempBody);
            archive.cuthtml =  tempBody.length < 5 ? '' : mmd.render(sliceBody);
        ...
        pageFiles.forEach(function(item) {
            ...
            if (stat.isFile() && item.indexOf('.markdown') >= 0) {
                page = loadPage(item);
                page._file = item;
                page.html = mmd.render(page.body);
